...
- preview 표시와 외부 result publisher 호출 타이밍을 관리한다
...
3. 필요하면 `ResultPublisher` 구현체를 붙인다
4. `FrameProcessingRunner`에 꽂아서 실행한다

즉 새 앱은 공통 루프를 다시 만들지 말고, processor와 publisher만 바꿔 끼워서 쓰면 된다.
