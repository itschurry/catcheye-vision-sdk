...
{
    auto rotate_left = [](std::uint32_t value, int bits) {
...
WebSocketPublisher::WebSocketPublisher(WebSocketPublisherConfig config)
...
bool WebSocketPublisher::start()
{
    ...
    addr.sin_addr.s_addr = config_.bind_address == "0.0.0.0"
        ? INADDR_ANY
        : inet_addr(config_.bind_address.c_str());
...
void WebSocketPublisher::publish(const catcheye::protocol::FrameMessage& message, const PublishContext& context)
{
    if (!running_ || message.empty()) {
        return;
    }

    const std::string metadata = build_metadata_frame(message, context);
    const auto metadata_frame = websocket_frame(
        std::span<const std::uint8_t>(
            reinterpret_cast<const std::uint8_t*>(metadata.data()),
            metadata.size()),
        0x1U);
    const auto binary_frame = websocket_frame(message.payload, 0x2U);

    std::lock_guard<std::mutex> lock(clients_mutex_);
    auto it = client_fds_.begin();
    while (it != client_fds_.end()) {
        const bool ok =
            send_all(*it, metadata_frame.data(), metadata_frame.size())
            && send_all(*it, binary_frame.data(), binary_frame.size());
        if (!ok) {
